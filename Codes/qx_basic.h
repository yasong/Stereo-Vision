...
inline int qx_square_difference_u3(unsigned char *a,unsigned char *b){int d1,d2,d3; d1=(*a++)-(*b++); d2=(*a++)-(*b++);	d3=(*a++)-(*b++); return(int(sqrt(d1*d1+d2*d2+d3*d3)));} //change into sqrt()
inline int qx_difference_u3(unsigned char *a,unsigned char *b){int d1,d2,d3; d1=(*a++)-(*b++); d2=(*a++)-(*b++);	d3=(*a++)-(*b++); return(int(sqrt(abs(d1)+abs(d2)+abs(d3))));} //change into sqrt()
